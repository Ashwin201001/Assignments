...
import '../styles/Userviewdevices.css';
...
const UserViewDevices = () => {
  ...
  return (
    <div className="device-container mt-5">
  <h2 className="device-heading mb-4">View Devices</h2>
  <div className="device-search mb-3">
    <select
      className="device-select form-select"
      value={searchType}
    ...
    <input
      type="text"
      className="device-input form-control mt-2"
      placeholder={`Enter ${searchType === 'name' ? 'Device Name' : searchType === 'status' ? 'Status' : 'Device Type'}`}
      value={searchTerm}
      onChange={(e) => setSearchTerm(e.target.value)}
      onKeyPress={handleKeyPress} // Call handleKeyPress on key press
    />
    <button className="device-btn btn btn-primary mt-2" onClick={handleSearch} style={{ backgroundColor: "rgb(132, 194, 215)", border: "#26A69A" }}>
      Search
    </button>
  </div>
  {errorState && (
    <div className="device-alert alert alert-danger" role="alert">
      {errorMessage}
    </div>
  )}
  <table className="device-table table mt-4">
    <thead>
      <tr>
        <th>Name</th>
        <th>Type</th>
    ...
    <tbody>
      {devices.map(device => (
        <tr key={device.deviceId}>
          ...
          <td>
            <button
              className="device-action btn btn-sm btn-primary"
              onClick={() => handleReplacementRequest(device)} style={{ backgroundColor: "rgb(132, 194, 215)", border: "#26A69A" }}
            >
              Request Replacement
            </button>
          </td>
